...
sealed class CameraException(message: String = "") :
    IllegalStateException(message) {

    class CameraInUseException :
        CameraException("CameraDevice is already in use by someone high-priority.")
    ...
    class CameraIsDisabledException :
        CameraException("CameraDevice is disabled by administrator.")
    ...
    class CameraPermissionRequired :
        CameraException("CameraDevice permission required")
}

sealed class SessionException(message: String) :
    IllegalStateException(message) {

    class ConfigureFailedException :
        SessionException("CaptureSession configuration is failed")
...
fun cameraExceptionByErrorCode(errorCode: Int): CameraException =
    when (errorCode) {
